...
varmin=[-j,-j,-j,-j];
varmax=[j,j,j,j];
MaxIt=1;%%%%%%%%%generations=MaxIt%%%%%in this program%%%%%%
npop=10;
w=1;
wdamp=0.99;
c1=1.5;
 ...
 for it=1:MaxIt
    for i=1:npop
     ...
     particle(i).position=particle(i).position+particle(i).velocity;
      for k= 1:4
     if( particle(i).position(1,k)>=exp(j*pi/4))&&(particle(i).position(1,k) <exp(3*j*pi/4))
        particle(i).position(1,k)=j;
     else if( particle(i).position(1,k)>=exp(3*j*pi/4))&&(particle(i).position(1,k)<exp(5*j*pi/4))
         particle(i).position(1,k)=-1;
         else if (particle(i).position(1,k))>=exp(5*j*pi/4)&&(particle(i).position(1,k)<exp(7*j*pi/4))
              particle(i).position(1,k)=-j;
             else
                particle(i).position(1,k)=1;
             end
